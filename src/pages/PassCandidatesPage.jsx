...
const PassCandidatesPage = () => {
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        const response = await axios.get('http://localhost:8000/pass-candidates', {
          params: {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
        <Typography variant="h5">Pass Candidates</Typography>
        <button
          onClick={() => navigate('/')}
          style={{
            padding: '8px 16px',
...
export default PassCandidatesPage;
